...
Write-Host "Provide the virtual machine Tags" -ForegroundColor Blue
...
Write-Output " "  
	foreach($oC in $Tags){  
       Write-Output "[$countTag] $oc"  
       $countTag = $countTag+1  
       }  
$choice = Read-Host "Which Tag do you want to review?"  
Write-Output " "  
...
foreach ($vm in $vms){ 
		...
		$resultObject | add-member -membertype NoteProperty -name "Storage Tag" -Value $TagEntity.Tag.Name
